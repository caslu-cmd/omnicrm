...
export default function TeamPortalPage() {
  ...
  const loadContacts = async () => {
    ...
    const { data } = await (supabase as any)
      .from("contacts")
      .select("id, name, email, phone, company, source, score, stage, created_at")
      .eq("client_id", clientId)
  ...
  const roleColor = member?.role === "financeiro" ? "#FBBF24" : accent;
  const RoleIcon  = member?.role === "financeiro" ? Crown : TrendingUp;
      ...
      <div style={{ background: "rgba(255,255,255,0.03)", borderBottom: "1px solid rgba(255,255,255,0.07)", padding: "0 24px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", height: 60, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <span style={{ fontWeight: 700, fontSize: 15 }}>Calu Agência</span>
          </div>

          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            {member && (
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <div style={{ width: 28, height: 28, borderRadius: "50%", background: `${roleColor}15`, border: `1px solid ${roleColor}30`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <RoleIcon style={{ width: 14, height: 14, color: roleColor }} />
                </div>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ fontSize: 20, fontWeight: 700, color: "rgba(255,255,255,0.9)", margin: 0 }}>
            {member?.role === "financeiro" ? "Relatório financeiro" : "CRM — Leads e Contatos"}
          </h1>
            ...
            {filtered.map(c => {
              ...
              return (
                <div key={c.id} style={{ display: "grid", gridTemplateColumns: "2fr 1.5fr 1.2fr 1fr 100px", gap: 12, padding: "14px 16px", borderRadius: 14, background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.06)", alignItems: "center" }}>
                  <div style={{ fontWeight: 600, fontSize: 13, color: "rgba(255,255,255,0.85)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                    {c.name || "—"}
                  </div>
                  ...
                  <div>
                    {src ? (
                      <span style={{ fontSize: 10, padding: "3px 8px", borderRadius: 20, background: `${src.color}15`, color: src.color, fontWeight: 600 }}>
                        {src.label}
                      </span>
