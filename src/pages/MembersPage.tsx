...
import {
  ...
  BarChart3, MoreVertical, ChevronRight, Clock, Star, BookOpen
} from "lucide-react";
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="p-6 space-y-6">
      <motion.div variants={item} className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold font-display text-foreground">Área de Membros</h1>
      ...
      <motion.div variants={item} className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        {[
          ...
          { label: "Taxa Conclusão", value: `${Math.round(courses.filter(c => c.status === "published").reduce((a, b) => a + b.completion, 0) / courses.filter(c => c.status === "published").length)}%`, icon: BarChart3, color: "secondary" },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-3">
          {courses.map(c => (
            <motion.div
              ...
              className={cn(
                "flex items-center gap-4 rounded-xl border bg-card p-5 shadow-card hover:shadow-elevated transition-all cursor-pointer",
                selectedCourse === c.id ? "border-primary/30" : "border-border"
              )}
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className={cn("px-2 py-0.5 rounded-md text-[11px] font-medium", c.status === "published" ? "bg-secondary/10 text-secondary" : "bg-muted text-muted-foreground")}>
                ...
                {c.status === "published" && (
                  <div className="flex items-center gap-2 mt-2">
                    <div className="h-1.5 flex-1 max-w-32 rounded-full bg-muted overflow-hidden">
                      <div className="h-full rounded-full bg-secondary" style={{ width: `${c.completion}%` }} />
                    </div>
        ...
        <div className="rounded-xl border border-border bg-card shadow-card p-5 space-y-4 overflow-y-auto scrollbar-thin">
          <h3 className="text-sm font-semibold text-foreground">
            {selectedCourse ? courses.find(c => c.id === selectedCourse)?.title : "Selecione um curso"}
          </h3>
          {selectedCourse && (
            <div className="space-y-2">
              {sampleModules.map((m, i) => (
                <div key={m.id} className={cn("flex items-center gap-3 p-3 rounded-lg border transition-colors", m.locked ? "border-border bg-muted/30 opacity-60" : "border-border hover:bg-muted/50")}>
                  <div className={cn("flex h-8 w-8 items-center justify-center rounded-lg text-xs font-bold", m.locked ? "bg-muted text-muted-foreground" : "bg-primary/10 text-primary")}>
                    {m.locked ? <Lock className="h-3.5 w-3.5" /> : i + 1}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium text-foreground truncate">{m.title}</p>
                    <p className="text-[10px] text-muted-foreground flex items-center gap-2">
                      <span>{m.lessons} aulas</span>
                      <span className="flex items-center gap-0.5"><Clock className="h-2.5 w-2.5" /> {m.duration}</span>
                    </p>
                  </div>
