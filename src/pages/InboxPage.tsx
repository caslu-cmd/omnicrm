...
const channelColor: Record<string, string> = {
  ...
  email: "text-primary",
  instagram: "text-accent-foreground",
    ...
    <div className="flex h-full">
        ...
        <div className="p-4 border-b border-border space-y-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-lg border border-input bg-background py-2 pl-10 pr-4 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring/20"
            />
          </div>
          <div className="flex gap-2">
            {["Todos", "WhatsApp", "E-mail", "Instagram"].map((f) => (
              <button key={f} className="px-2.5 py-1 rounded-md text-xs font-medium bg-muted text-muted-foreground hover:bg-primary/10 hover:text-primary transition-colors">
                {f}
              </button>
            ))}
          </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 mt-1.5">
                    {c.tags.map((t) => (
                      <span key={t} className="px-1.5 py-0.5 rounded text-[10px] font-medium bg-primary/10 text-primary">{t}</span>
                    ))}
                    {c.unread > 0 && (
                      <span className="ml-auto flex h-5 min-w-5 items-center justify-center rounded-full bg-primary text-[11px] font-bold text-primary-foreground">
                        {c.unread}
                      </span>
                    )}
                  </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground flex items-center gap-1">
                <span className="h-2 w-2 rounded-full bg-secondary inline-block" /> Online · via {conv.channel}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button className="p-2 rounded-lg hover:bg-muted text-muted-foreground"><Star className="h-4 w-4" /></button>
            <button className="p-2 rounded-lg hover:bg-muted text-muted-foreground"><Tag className="h-4 w-4" /></button>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin p-6 space-y-4 bg-background">
          {messages.map((m) => (
            <motion.div
              key={m.id}
              initial={{ opacity: 0, x: m.from === "client" ? -10 : 10 }}
            ...
            >
              <div
                className={cn(
                  "max-w-md rounded-2xl px-4 py-3 text-sm whitespace-pre-line",
                  m.from === "agent"
              ...
              >
                {m.text}
                <p className={cn("text-[10px] mt-1.5", m.from === "agent" ? "text-primary-foreground/60" : "text-muted-foreground")}>
                  {m.time}
                </p>
              </div>
            </motion.div>
          ...
          <div className="flex items-end gap-3">
            <div className="flex-1 rounded-xl border border-input bg-background p-3">
              <textarea
                ...
                rows={1}
                className="w-full resize-none text-sm bg-transparent placeholder:text-muted-foreground focus:outline-none"
              />
              <div className="flex items-center gap-2 mt-2">
                <button className="p-1.5 rounded-md hover:bg-muted text-muted-foreground"><Paperclip className="h-4 w-4" /></button>
                <button className="p-1.5 rounded-md hover:bg-muted text-muted-foreground"><Smile className="h-4 w-4" /></button>
              </div>
            </div>
            <button className="flex h-11 w-11 items-center justify-center rounded-xl bg-primary text-primary-foreground hover:bg-primary/90 transition-colors active:scale-95">
      ...
      <div className="w-72 border-l border-border bg-card p-5 space-y-5 overflow-y-auto scrollbar-thin hidden xl:block">
        <div className="text-center">
          <div className="flex h-16 w-16 mx-auto items-center justify-center rounded-full bg-primary/10 text-lg font-bold text-primary">
            {conv.avatar}
          </div>
        ...
        <div className="space-y-3">
          ...
          {[
            ...
            { label: "Pipeline", value: "Qualificação" },
          ].map((info) => (
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {["Lead Quente", "Demo Agendada", "Pro"].map((t) => (
              <span key={t} className="px-2 py-1 rounded-md text-xs font-medium bg-primary/10 text-primary">{t}</span>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-lg bg-muted/50 p-3 text-sm text-muted-foreground">
            Interessada no plano Pro. Equipe de 5 vendedores. Agendar demo para amanhã.
          </div>
        </div>
      </div>
    </div>
