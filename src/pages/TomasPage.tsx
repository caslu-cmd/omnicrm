...
function getSectionColumns(markedHtml: string, sectionId: string): SectionColumn[] {
  const doc = new DOMParser().parseFromString(markedHtml, "text/html");
  const sec = doc.querySelector(`[data-calu-section="${sectionId}"]`);
  if (!sec) return [{ index: 0, hasText: true, hasImage: false, hasForm: false, isEmpty: false, preview: "" }];

  let colEls: Element[] = [];
  const gridEl = sec.querySelector('[class*="grid-cols"]');
  if (gridEl) colEls = Array.from(gridEl.children).filter(el => el.tagName !== "SCRIPT" && el.tagName !== "STYLE");
  ...
  if (colEls.length < 2) {
    ...
    if (flexEl) {
      ...
      if (kids.length >= 2) colEls = kids;
    }
  }
  if (colEls.length < 2 && sec.children.length === 1) {
    const wrapper = sec.children[0];
    const kids = Array.from(wrapper.children).filter(el => (el.textContent?.trim().length ?? 0) > 0 || el.querySelector("img"));
    if (kids.length >= 2) colEls = kids;
  }

  if (colEls.length < 2) {
    const hasImage = !!sec.querySelector("img") || sec.innerHTML.includes("background-image");
    const hasForm = sec.innerHTML.toLowerCase().includes("<form") || sec.innerHTML.toLowerCase().includes("forminator") || sec.innerHTML.includes("[contact");
    return [{ index: 0, hasText: !!sec.querySelector("h1,h2,h3,p"), hasImage, hasForm, isEmpty: false, preview: sec.textContent?.trim().slice(0, 50) ?? "" }];
  }

  return colEls.map((col, i) => ({
    index: i,
    hasText: !!col.querySelector("h1,h2,h3,h4,p,li,span"),
    hasImage: !!col.querySelector("img") || col.innerHTML.includes("background-image"),
    hasForm: col.innerHTML.toLowerCase().includes("<form") || col.innerHTML.toLowerCase().includes("forminator") || col.innerHTML.includes("[contact"),
    isEmpty: (col.textContent?.trim().length ?? 0) === 0 && !col.querySelector("img"),
    preview: col.textContent?.trim().slice(0, 45) ?? "",
  }));
}

function applyBlockInsertInColumn(html: string, sectionId: string, columnIndex: number, blockHtml: string): string {
  const doc = new DOMParser().parseFromString(html, "text/html");
  const sec = doc.querySelector(`[data-calu-section="${sectionId}"]`);
  if (!sec) return html;
  ...
  if (gridEl) {
    ...
    targetEl = cols[columnIndex] ?? cols[cols.length - 1] ?? sec;
  }

  const div = doc.createElement("div");
  div.innerHTML = blockHtml;
  while (div.firstChild) targetEl.appendChild(div.firstChild);
  return "<!DOCTYPE html>\n" + doc.documentElement.outerHTML;
}
...
function applyAddColumnToGrid(html: string, sectionId: string, gridIdx: number): string {
  ...
  const sec = doc.querySelector(`[data-calu-section="${sectionId}"]`);
  if (!sec) return html;
  // Collect both Tailwind class-based and inline-style grids (mirrors iframe findSectionGrids)
  const seen = new Set<Element>();
  const grids: Element[] = [];
  const add = (el: Element) => { if (el.children.length >= 2 && !seen.has(el)) { seen.add(el); grids.push(el); } };
  sec.querySelectorAll('[class*="grid-cols"]').forEach(add);
  sec.querySelectorAll('[style]').forEach(el => { if (/grid-template-columns/i.test(el.getAttribute('style') ?? '')) add(el); });
  ...
  // Bump Tailwind grid-cols-N class
  const clsMatch = grid.className.match(/grid-cols-(\d+)/);
  if (clsMatch) {
    const n = Math.min(parseInt(clsMatch[1]) + 1, 6);
    grid.className = grid.className.replace(/grid-cols-\d+/, `grid-cols-${n}`);
  }
  // Bump inline-style grid-template-columns
  const style = grid.getAttribute("style") ?? "";
  if (style) {
    const repeatMatch = style.match(/grid-template-columns:\s*repeat\((\d+)/i);
    if (repeatMatch) {
      const n = Math.min(parseInt(repeatMatch[1]) + 1, 6);
      grid.setAttribute("style", style.replace(/grid-template-columns:\s*repeat\(\d+/i, `grid-template-columns:repeat(${n}`));
    } else if (/grid-template-columns/i.test(style)) {
      grid.setAttribute("style", style.replace(/grid-template-columns:\s*([^;]+)/i, (_: string, cols: string) => `grid-template-columns:${cols.trim()} 1fr`));
    }
  }
  return "<!DOCTYPE html>\n" + doc.documentElement.outerHTML;
}
...
function applyRemoveColumns(html: string, sectionId: string): string {
  ...
  const el = doc.querySelector(`[data-calu-section="${sectionId}"]`);
  if (!el) return html;
  const gridEl = el.querySelector('[class*="grid-cols"]');
  if (!gridEl) return html;
  const frag = doc.createDocumentFragment();
  Array.from(gridEl.children).forEach(child => frag.appendChild(child.cloneNode(true)));
  gridEl.replaceWith(frag);
  return "<!DOCTYPE html>\n" + doc.documentElement.outerHTML;
}
...
bs.textContent=
  ...
  '#calu-sec-bar .btn-col{background:rgba(180,120,255,0.13);color:#C89FFF;min-width:30px;justify-content:center;padding:4px 7px;font-size:10px;}#calu-sec-bar .btn-col:hover{background:rgba(180,120,255,0.28);}'+
  '#calu-sec-bar .btn-col.active{background:rgba(185,255,75,0.22)!important;color:#B9FF4B!important;box-shadow:0 0 0 1px rgba(185,255,75,0.4);}'+
...
var colBtns=[1,2,3,4].map(function(n){
  ...
  b.title=n+(n===1?' coluna':' colunas');
  b.setAttribute('data-cn',String(n));
  bar.appendChild(b);
  return b;
});
...
function getSecCols(sec){
  if(sec.querySelector('[class*="grid-cols-4"]'))return 4;
  if(sec.querySelector('[class*="grid-cols-3"]'))return 3;
  if(sec.querySelector('[class*="grid-cols-2"]'))return 2;
  return 1;
}
...
function posBar(sec){
  ...
  bar.style.right=(document.documentElement.clientWidth-r.right+8)+'px';
  bar.style.left='auto';
  bar.style.display='flex';
  var active=getSecCols(sec);
  colBtns.forEach(function(b){
    if(parseInt(b.getAttribute('data-cn'))===active){b.classList.add('active');}
...
acs.textContent='#calu-add-col-btn{position:fixed;z-index:2147483644;display:none;align-items:center;justify-content:center;width:28px;height:28px;border-radius:50%;background:#B9FF4B;color:#07080A;font-size:20px;font-weight:900;border:none;cursor:pointer;box-shadow:0 2px 12px rgba(0,0,0,.7);transition:transform .15s,opacity .15s;}#calu-add-col-btn:hover{transform:scale(1.18);}';
document.head.appendChild(acs);
acb.innerHTML='+';acb.title='Adicionar coluna';
document.body.appendChild(acb);
var curGridSid=null,curGridIdx=null,acbTimer=null;
// Detect grids: class-based (Tailwind) OR inline style grid-template-columns
function findSectionGrids(sec){
  var seen=new Set();var list=[];
  function add(el){if(el.children.length>=2&&!seen.has(el)){seen.add(el);list.push(el);}}
  sec.querySelectorAll('[class*="grid-cols"]').forEach(add);
  sec.querySelectorAll('[style]').forEach(function(el){
    var s=el.getAttribute('style')||'';
    if(/grid-template-columns/i.test(s))add(el);
  });
  return list;
}
document.querySelectorAll('[data-calu-section]').forEach(function(sec){
  var sid=sec.getAttribute('data-calu-section');
  var grids=findSectionGrids(sec);
  grids.forEach(function(grid,gi){
    grid.addEventListener('mouseenter',function(){
      if(acbTimer)clearTimeout(acbTimer);
      curGridSid=sid;curGridIdx=gi;
      var r=grid.getBoundingClientRect();
      acb.style.top=(r.top+r.height/2-14)+'px';
      acb.style.left=Math.min(r.right+8,document.documentElement.clientWidth-34)+'px';
      acb.style.display='flex';
    });
    grid.addEventListener('mouseleave',function(){
      acbTimer=setTimeout(function(){if(!acb.matches(':hover')){acb.style.display='none';}},180);
    });
  });
});
acb.addEventListener('mouseenter',function(){if(acbTimer)clearTimeout(acbTimer);});
acb.addEventListener('mouseleave',function(){acb.style.display='none';});
...
export default function TomasPage() {
  ...
  const [libraryPages, setLibraryPages]   = useState<LPRow[]>([]);
  const [libraryLoading, setLibraryLoading] = useState(false);
  const [lpSearch, setLpSearch]           = useState("");
  const [renamingLpId, setRenamingLpId]   = useState<string | null>(null);
  const [renameLpValue, setRenameLpValue] = useState("");
  ...
  useEffect(() => {
    const handler = (e: MessageEvent) => {
      ...
      if (e.data?.type === "calu-section-columns") {
        ...
        setMarkedHtml(prev => {
          const newHtml = cols === 1
            ? applyRemoveColumns(prev, sectionId)
            : applySplitColumns(prev, sectionId, cols as 2 | 3 | 4);
          const { markedHtml: nm, sections: ns } = parseLPIntoSections(newHtml);
          setSections(ns);
          setHtmlEditado(stripEditorAttrs(newHtml));
          return nm;
        });
        toast.success(cols === 1 ? "Colunas removidas!" : `Seção dividida em ${cols} colunas!`);
      }
  ...
  const handleApplyBlockInsert = () => {
    ...
    if (blockType === "image") {
      ...
      if (!src) { toast.error("Informe a URL ou faça upload da imagem"); return; }
      blockHtml = `<div style="padding:12px 0;text-align:center"><img src="${src}" alt="" style="max-width:100%;height:auto;border-radius:8px;" /></div>`;
    } else if (blockType === "form") {
      if (!blockVal.trim()) { toast.error("Cole o código do formulário"); return; }
      blockHtml = `<div class="block-form" style="padding:16px 0;">${blockVal}</div>`;
    } else if (blockType === "html") {
      if (!blockVal.trim()) { toast.error("Cole o HTML"); return; }
      blockHtml = blockVal;
    } else if (blockType === "spacer") {
      const px = parseInt(blockVal) || 48;
      blockHtml = `<div style="height:${px}px;display:block;"></div>`;
    }
...
<script>(function(){
  ...
  function renderEditorPanel() {
    return (
      <div className="flex flex-col h-full" style={{ background: "#0A0A10" }}>
        ...
        {/* Header */}
        <div className="flex flex-col border-b flex-shrink-0" style={{ borderColor: "#1E1E2E" }}>
        ...
        {templatesOpen ? (
          ...
          /* ─── BLOCOS MODE ─────────────────────────────────────────────── */
          <div className="flex-1 overflow-y-auto px-3 py-3 flex flex-col gap-3">
            <input ref={blockImgRef} type="file" accept="image/*" className="hidden" onChange={handleBlockImageFile} />

            {/* Insert before first section */}
            <button onClick={() => { setBlockInsertTarget({ sectionId: sections[0].id, colIdx: 0, mode: "before" }); setBlockType(null); setBlockVal(""); setBlockImgPrev(""); }}
              className="w-full flex items-center justify-center gap-1.5 py-1.5 rounded-xl text-[11px] transition-all border-dashed"
              style={{ background: "transparent", border: "1px dashed #2A2A3A", color: "#444466" }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = "#B9FF4B44"; e.currentTarget.style.color = "#B9FF4B"; }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = "#2A2A3A"; e.currentTarget.style.color = "#444466"; }}>
              + inserir seção aqui
            </button>
            ...
            {sections.map((sec, sIdx) => {
              ...
              return (
                <div key={sec.id} className="flex flex-col gap-1.5"
                  ...
                  <button onClick={() => { setBlockInsertTarget({ sectionId: sec.id, colIdx: 0, mode: "after" }); setBlockType(null); setBlockVal(""); setBlockImgPrev(""); }}
                    className="w-full flex items-center justify-center gap-1.5 py-1 rounded-xl text-[11px] transition-all"
                    style={{ background: "transparent", border: "1px dashed #2A2A3A", color: "#333355" }}
                    onMouseEnter={e => { e.currentTarget.style.borderColor = "#B9FF4B44"; e.currentTarget.style.color = "#B9FF4B"; }}
                    onMouseLeave={e => { e.currentTarget.style.borderColor = "#2A2A3A"; e.currentTarget.style.color = "#333355"; }}>
                    + inserir seção após
                  </button>
                </div>
              );
            ...
            {blockInsertTarget && (
              <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center" style={{ background: "rgba(0,0,0,0.7)" }}
                onClick={e => { if (e.target === e.currentTarget) { setBlockInsertTarget(null); setBlockType(null); } }}>
                <div className="w-full max-w-sm rounded-2xl overflow-hidden" style={{ background: "#0E0E18", border: "1px solid #2A2A3A", maxHeight: "85vh", overflowY: "auto" }}>
                  ...
                  <div className="p-4 flex flex-col gap-4">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        { id: "image" as const, label: "Imagem", icon: "🖼️", desc: "Upload ou URL" },
                        { id: "form"  as const, label: "Formulário", icon: "📋", desc: "Cole o HTML/embed" },
                        { id: "html"  as const, label: "HTML livre", icon: "</>", desc: "Código personalizado" },
                        { id: "spacer" as const, label: "Espaço", icon: "↕️", desc: "Separador de altura" },
                      ].map(t => (
                        <button key={t.id} onClick={() => { setBlockType(t.id); setBlockVal(""); setBlockImgPrev(""); }}
                          className="flex flex-col items-center gap-1 p-3 rounded-xl transition-all text-center"
                          style={{ background: blockType === t.id ? "#B9FF4B18" : "#141420", border: `1px solid ${blockType === t.id ? "#B9FF4B55" : "#2A2A3A"}`, color: blockType === t.id ? "#B9FF4B" : "rgba(255,255,255,0.5)" }}>
                          <span className="text-lg">{t.icon}</span>
                          <span className="text-xs font-semibold">{t.label}</span>
                          <span className="text-[10px]" style={{ color: "rgba(255,255,255,0.3)" }}>{t.desc}</span>
                        </button>
                      ))}
                    </div>
                    ...
                    {blockType === "image" && (
                      <div className="flex flex-col gap-3">
                        <div className="flex gap-2">
                          <button onClick={() => blockImgRef.current?.click()}
                            className="flex items-center gap-2 px-3 py-2 rounded-xl text-sm transition-all flex-1"
                            style={{ background: "#141420", border: "1px solid #2A2A3A", color: "rgba(255,255,255,0.7)" }}>
                            <Upload className="w-4 h-4" /> {blockUploading ? "Carregando..." : "Upload"}
                          </button>
                        </div>
                        {blockImgPrev && <img src={blockImgPrev} alt="preview" className="w-full rounded-lg max-h-32 object-contain" style={{ background: "#141420" }} />}
                        <div>
                          <p className="text-[11px] mb-1.5" style={{ color: "#555577" }}>Ou cole uma URL:</p>
                          <input type="url" placeholder="https://..." value={blockImgPrev ? "" : blockVal}
                            onChange={e => { setBlockVal(e.target.value); setBlockImgPrev(""); }}
                    ...
                    {blockType === "form" && (
                      <div className="flex flex-col gap-2">
                        <p className="text-[11px]" style={{ color: "#555577" }}>Cole o código HTML do formulário (Forminator, Elementor Forms, WPForms, ou qualquer iframe/shortcode convertido em HTML):</p>
                        <textarea rows={6} value={blockVal} onChange={e => setBlockVal(e.target.value)}
                          placeholder={'<form action="...">\n  <input type="email" />\n  <button>Enviar</button>\n</form>'}
                          className="w-full resize-none rounded-xl px-3 py-2.5 text-xs font-mono outline-none"
                          style={{ background: "#0D0D16", border: "1px solid #2A2A3A", color: "#E0E0F0", lineHeight: 1.6 }} />
                      </div>
                    )}
                    ...
                    {blockType === "html" && (
                      <div className="flex flex-col gap-2">
                        <p className="text-[11px]" style={{ color: "#555577" }}>Cole qualquer HTML — vídeo, contador, mapa, widget:</p>
                        <textarea rows={6} value={blockVal} onChange={e => setBlockVal(e.target.value)}
                    ...
                    {blockType === "spacer" && (
                      <div className="flex flex-col gap-2">
                        <p className="text-[11px]" style={{ color: "#555577" }}>Altura em pixels:</p>
                        <input type="number" min={8} max={300} value={blockVal || "48"} onChange={e => setBlockVal(e.target.value)}
                          className="w-full rounded-xl px-3 py-2 text-sm outline-none"
                          style={{ background: "#141420", border: "1px solid #2A2A3A", color: "#E0E0F0" }} />
                      </div>
                    )}
                    ...
                    {/* Apply button */}
                    {blockType && (
                      <button onClick={handleApplyBlockInsert}
                        className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold transition-all"
                        style={{ background: "#B9FF4B", color: "#07080A" }}>
                        <Check className="w-4 h-4" /> Inserir bloco
                      </button>
                    )}
                  </div>
                </div>
  ...
  const fetchLibrary = useCallback(async () => {
    ...
    try {
      ...
      let q = (supabase as any).from("landing_pages")
  ...
  const openLpFromLibrary = useCallback(async (lp: LPRow) => {
    setLibraryLoading(true);
    const { data } = await (supabase as any).from("landing_pages").select("html_content").eq("id", lp.id).single();
    setLibraryLoading(false);
    if (!data?.html_content) { toast.error("Não foi possível carregar a LP"); return; }
    setResultado({ copy: "", design: "", html: data.html_content });
    setHtmlEditado(data.html_content);
    setProduto(lp.title);
  ...
  const deleteLpFromLibrary = useCallback(async (id: string) => {
    await (supabase as any).from("landing_pages").delete().eq("id", id);
    setLibraryPages(prev => prev.filter(p => p.id !== id));
    if (savedPageId === id) setSavedPageId(null);
    toast.success("LP excluída!");
  }, [savedPageId]);

  const saveLpRename = useCallback(async (id: string) => {
    const title = renameLpValue.trim();
    if (!title) { setRenamingLpId(null); return; }
    await (supabase as any).from("landing_pages").update({ title }).eq("id", id);
    setLibraryPages(prev => prev.map(p => p.id === id ? { ...p, title } : p));
    setRenamingLpId(null);
    toast.success("Título atualizado!");
  }, [renameLpValue]);

  const duplicateLpInLibrary = useCallback(async (lp: LPRow) => {
    const { data: orig } = await (supabase as any).from("landing_pages").select("html_content").eq("id", lp.id).single();
    if (!orig?.html_content) return;
    const { data: { session } } = await supabase.auth.getSession();
    const uid = session?.user?.id;
    const { data: dup } = await (supabase as any).from("landing_pages").insert({
      title: `Cópia de ${lp.title}`,
      slug: `copia-${Date.now()}`,
      html_content: orig.html_content,
      status: "draft",
      created_by: uid ?? null,
    }).select("id, title, slug, status, updated_at, created_at").single();
    if (dup) setLibraryPages(prev => [dup, ...prev]);
    toast.success("LP duplicada!");
  }, []);
          ...
          <>
            ...
            <div className="flex items-center gap-3 px-5 py-4 border-b" style={{ borderColor: "#1E1E2E", background: "#0A0A10" }}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[11px] truncate" style={{ color: "#B9FF4B", opacity: 0.7 }}>
                  {clientName ? `LP para ${clientName}` : "Criador de Landing Pages"}
                </div>
              </div>
              <button
                onClick={() => { const next = !libraryOpen; setLibraryOpen(next); if (next) fetchLibrary(); }}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold transition-all flex-shrink-0"
                style={{ background: libraryOpen ? "#B9FF4B22" : "#141420", border: `1px solid ${libraryOpen ? "#B9FF4B55" : "#2A2A3A"}`, color: libraryOpen ? "#B9FF4B" : "rgba(255,255,255,0.4)" }}>
                <Layout className="w-3.5 h-3.5" />
                {libraryOpen ? "Nova LP" : `Minhas LPs${libraryPages.length > 0 ? ` (${libraryPages.length})` : ""}`}
              </button>
            </div>
            ...
            {libraryOpen && (
              <div className="flex-1 flex flex-col overflow-hidden" style={{ background: "#0A0A10" }}>
                ...
                <div className="px-4 pt-3 pb-2 flex-shrink-0">
                  <div className="flex items-center gap-2 px-3 py-2 rounded-xl" style={{ background: "#141420", border: "1px solid #2A2A3A" }}>
                    <Search className="w-3.5 h-3.5 flex-shrink-0" style={{ color: "#555577" }} />
                    <input value={lpSearch} onChange={e => setLpSearch(e.target.value)}
                ...
                <div className="flex-1 overflow-y-auto px-3 pb-4 flex flex-col gap-2">
                  {libraryLoading ? (
                    <div className="flex items-center justify-center py-10">
                      <Loader2 className="w-5 h-5 animate-spin" style={{ color: "#B9FF4B" }} />
                    </div>
                  ) : libraryPages.filter(p => p.title.toLowerCase().includes(lpSearch.toLowerCase())).length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-12 gap-3">
                      <Layout className="w-8 h-8" style={{ color: "#2A2A3A" }} />
                      <p className="text-xs text-center" style={{ color: "#444466" }}>
                        {lpSearch ? "Nenhuma LP encontrada" : "Nenhuma LP salva ainda.\nGere sua primeira e ela aparece aqui!"}
                      </p>
                    </div>
                  ) : (
                    libraryPages
                      .filter(p => p.title.toLowerCase().includes(lpSearch.toLowerCase()))
                      .map(lp => (
                        <div key={lp.id} className="group rounded-xl p-3 flex flex-col gap-2 transition-all"
                          style={{ background: "#0D0D18", border: "1px solid #1E1E2E" }}
                          ...
                          <div className="flex items-start gap-2">
                            {renamingLpId === lp.id ? (
                              <input
                                autoFocus
                                value={renameLpValue}
                                onChange={e => setRenameLpValue(e.target.value)}
                                onBlur={() => saveLpRename(lp.id)}
                                onKeyDown={e => { if (e.key === "Enter") saveLpRename(lp.id); if (e.key === "Escape") setRenamingLpId(null); }}
                                className="flex-1 text-xs font-semibold bg-transparent border-b outline-none"
                                style={{ color: "#F0F0F0", borderColor: "#B9FF4B55" }}
                              />
                            ) : (
                              <button
                                onClick={() => { setRenamingLpId(lp.id); setRenameLpValue(lp.title); }}
                                className="flex-1 text-left text-xs font-semibold truncate hover:text-white transition-colors"
                            ...
                            <span className="flex-shrink-0 text-[9px] px-1.5 py-0.5 rounded-full font-semibold"
                              style={{
                                background: lp.status === "published" ? "#B9FF4B22" : "#1E1E2E",
                                color: lp.status === "published" ? "#B9FF4B" : "#555577",
                                border: `1px solid ${lp.status === "published" ? "#B9FF4B44" : "#2A2A3A"}`,
                              }}>
                          ...
                          <p className="text-[10px]" style={{ color: "#333355" }}>
                            Editada {new Date(lp.updated_at).toLocaleDateString("pt-BR", { day: "2-digit", month: "short", year: "numeric" })}
                          </p>

                          {/* Actions */}
                          <div className="flex gap-1.5">
                            <button onClick={() => openLpFromLibrary(lp)}
                              className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-lg text-xs font-semibold transition-all"
                              style={{ background: "#B9FF4B", color: "#07080A" }}>
                              <Eye className="w-3 h-3" /> Abrir
                            </button>
                            <button onClick={() => duplicateLpInLibrary(lp)} title="Duplicar"
                              className="w-8 flex items-center justify-center rounded-lg transition-all"
                              style={{ background: "#141420", border: "1px solid #2A2A3A", color: "#8888BB" }}
                              onMouseEnter={e => e.currentTarget.style.color = "#B9FF4B"}
                              onMouseLeave={e => e.currentTarget.style.color = "#8888BB"}>
                              <Copy className="w-3.5 h-3.5" />
                            </button>
                            <button onClick={() => deleteLpFromLibrary(lp.id)} title="Excluir"
                              className="w-8 flex items-center justify-center rounded-lg transition-all"
                              style={{ background: "#141420", border: "1px solid #2A2A3A", color: "#555577" }}
                              onMouseEnter={e => e.currentTarget.style.color = "#FF4466"}
                              onMouseLeave={e => e.currentTarget.style.color = "#555577"}>
