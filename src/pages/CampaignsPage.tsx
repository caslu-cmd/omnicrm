    ...
    <motion.div variants={container} initial="hidden" animate="show" className="p-6 space-y-6">
      <motion.div variants={item} className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">{campaigns.length} campanhas · {avgOpenRate}% taxa de abertura média</p>
        </div>
        <button onClick={() => setTab("compose")} className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90 transition-colors">
        ...
        <>
          ...
          <motion.div variants={item} className="flex items-center gap-3">
            <div className="relative flex-1 max-w-sm">
              ...
              <input placeholder="Buscar campanhas..." className="w-full rounded-lg border border-input bg-card py-2 pl-10 pr-4 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring/20" />
            </div>
            <button className="flex items-center gap-2 px-3 py-2 rounded-lg border border-input bg-card text-sm font-medium text-muted-foreground hover:text-foreground"><Filter className="h-4 w-4" /> Filtros</button>
          </motion.div>
              ...
              return (
                <div key={c.id} className="rounded-xl border border-border bg-card p-5 shadow-card hover:shadow-elevated transition-shadow">
                  <div className="flex items-center gap-4">
                    <div className={cn("flex h-11 w-11 items-center justify-center rounded-xl", c.channel === "email" ? "bg-primary/10" : "bg-secondary/10")}>
                      <ChannelIcon className={cn("h-5 w-5", c.channel === "email" ? "text-primary" : "text-secondary")} />
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        {c.channel === "email" ? "E-mail" : "WhatsApp"} · {c.audience.toLocaleString()} destinatários · {c.sentAt}
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-1">
                      <button className="p-2 rounded-lg hover:bg-muted text-muted-foreground"><BarChart3 className="h-4 w-4" /></button>
          ...
          <div className="lg:col-span-2 space-y-5">
            <div className="rounded-xl border border-border bg-card p-5 shadow-card space-y-4">
                ...
                {(["email", "whatsapp"] as const).map(ch => (
                  <button
                    ...
                    className={cn(
                      ...
                      composeChannel === ch ? "border-primary bg-primary/5 text-primary" : "border-border text-muted-foreground hover:border-primary/30"
                    )}
                  >
              ...
              <div>
                ...
                <input placeholder="Ex: Newsletter Março 2026" className="w-full mt-1 rounded-lg border border-input bg-background py-2.5 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring/20" />
              </div>
              ...
              {composeChannel === "email" && (
                <>
                  <div>
                    ...
                    <input placeholder="Linha de assunto do e-mail" className="w-full mt-1 rounded-lg border border-input bg-background py-2.5 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring/20" />
                  </div>
                  <div>
                    <label className="text-xs font-medium text-muted-foreground">Conteúdo</label>
                    <textarea rows={8} placeholder="Compose seu e-mail aqui... Suporta HTML e variáveis como {{nome}}" className="w-full mt-1 rounded-xl border border-input bg-background py-3 px-4 text-sm focus:outline-none focus:ring-2 focus:ring-ring/20 resize-none" />
                  </div>
                </>
              )}
              ...
              {composeChannel === "whatsapp" && (
                <div>
                  ...
                  <select className="w-full mt-1 rounded-lg border border-input bg-background py-2.5 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring/20">
                    <option>Selecionar template...</option>
                    <option>{"Boas-vindas - Olá {{nome}}, bem-vindo!"}</option>
                    <option>{"Follow-up - {{nome}}, vimos seu interesse em..."}</option>
                    <option>{"Promoção - Oferta especial para você, {{nome}}"}</option>
                  </select>
                  <div className="mt-3 rounded-lg bg-muted/50 p-4 border border-border">
                    ...
                    <div className="bg-secondary/10 rounded-lg p-3 text-sm text-foreground">
                      Olá <span className="font-semibold text-primary">{'{{nome}}'}</span>, bem-vindo ao OmniCRM! 🎉<br />
                      Estamos felizes em ter você conosco. Precisa de ajuda?
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="flex gap-3">
              ...
              <button className="px-4 py-2.5 rounded-lg bg-muted text-sm font-medium text-foreground hover:bg-muted/80">Salvar Rascunho</button>
          ...
          <div className="space-y-5">
            <div className="rounded-xl border border-border bg-card p-5 shadow-card space-y-4">
              <h3 className="text-sm font-semibold text-foreground flex items-center gap-2">
                <Users className="h-4 w-4 text-primary" /> Audiência
              </h3>
              <select className="w-full rounded-lg border border-input bg-background py-2 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring/20">
                <option>Todos os contatos (12.450)</option>
                <option>Leads Quentes (2.340)</option>
                <option>Clientes Ativos (5.670)</option>
                <option>Inativos 30+ dias (1.560)</option>
                <option>Tags: Demo Agendada (890)</option>
              </select>
            ...
            <div className="rounded-xl border border-border bg-card p-5 shadow-card space-y-4">
              ...
              <div className="flex gap-2">
                <button className="flex-1 px-3 py-2 rounded-lg border border-primary bg-primary/5 text-sm font-medium text-primary">Agora</button>
                <button className="flex-1 px-3 py-2 rounded-lg border border-border text-sm font-medium text-muted-foreground hover:border-primary/30">Agendar</button>
              </div>
            </div>

            <div className="rounded-xl border border-border bg-card p-5 shadow-card space-y-4">
              <h3 className="text-sm font-semibold text-foreground flex items-center gap-2">
                <TrendingUp className="h-4 w-4 text-primary" /> A/B Testing
              </h3>
              <p className="text-xs text-muted-foreground">Teste variações de assunto ou conteúdo para otimizar resultados.</p>
