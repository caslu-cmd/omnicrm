...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ── OAuth callback: exchange code → long-lived token ─────────
    if (req.method === "POST" && action === "oauth-callback") {
      ...
      try {
        ...
        return respond({ error: "State inválido" }, 400);
      }
      const { clientId, platform } = stateData;

      // Short-lived token
      const tokenRes = await fetch(
        `${GRAPH}/oauth/access_token?client_id=${metaAppId}&redirect_uri=${encodeURIComponent(redirectUri)}&client_secret=${metaAppSecret}&code=${code}`
      );
      ...
      const longRes = await fetch(
        `${GRAPH}/oauth/access_token?grant_type=fb_exchange_token&client_id=${metaAppId}&client_secret=${metaAppSecret}&fb_exchange_token=${tokenData.access_token}`
      );
      ...
      if (!pages.length) return respond({ error: "Nenhuma Página do Facebook encontrada. Certifique-se de administrar ao menos uma Página." }, 400);
      ...
      const encryptedToken = obfuscate(tokenToStore, encKey);
    ...
    if (req.method === "GET" && action === "connections") {
      const clientId = url.searchParams.get("client_id");
      if (!clientId) return respond({ error: "client_id obrigatório" }, 400);
    ...
    // ── Create / publish / schedule post ────────────────────────
    if (req.method === "POST" && action === "create-post") {
      const { client_id, platforms, caption, media_url, media_type, scheduled_at } = await req.json();
        ...
        for (const platform of platforms as string[]) {
          ...
          try {
            if (platform === "facebook") {
              const postBody: Record<string, string> = { message: caption ?? "", access_token: accessToken };
              if (media_url && (media_type === "image" || !media_type)) {
                // Photo post
                const res = await fetch(`${GRAPH}/${conn.account_id}/photos`, {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({ url: media_url, caption: caption ?? "", access_token: accessToken }),
                });
                ...
                const result = await res.json();
                if (result.error) { errorMessage = result.error.message; status = "failed"; }
                else fbPostId = result.id;
              }
            } else if (platform === "instagram") {
              if (!media_url) { errorMessage = "Instagram requer uma imagem"; status = "failed"; continue; }
            ...
            errorMessage = e instanceof Error ? e.message : "Erro ao publicar";
            status = "failed";
          }
        }
    ...
    if (req.method === "DELETE" && action === "delete-post") {
      ...
      if (!postId) return respond({ error: "id obrigatório" }, 400);
      const { error } = await supabase
        .from("scheduled_posts")
        .delete()
        .eq("id", postId)
        ...
        try {
          ...
          if (conn.platform === "facebook") {
            const res = await fetch(
              `${GRAPH}/${conn.account_id}/insights?metric=page_impressions,page_reach,page_engaged_users&period=week&access_token=${accessToken}`
            );
            ...
            const res = await fetch(
              `${GRAPH}/${conn.account_id}/insights?metric=impressions,reach,profile_views&period=week&access_token=${accessToken}`
            );
            const d = await res.json();
            for (const item of d.data ?? []) {
              const v = item.values?.[item.values.length - 1]?.value ?? 0;
